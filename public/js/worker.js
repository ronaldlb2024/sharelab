/* public/js/worker.js
   Rodar pdf.js DENTRO do nosso Web Worker, sem worker interno do pdf.js.
   -> não setar workerSrc
   -> não carregar pdf.worker.min.js
   -> getDocument({ disableWorker: true })
*/

// util simples pra ver se algum import falhou
const DIAG = { imports: [] };
...
// 1) carregar SOMENTE pdf.min.js (sem pdf.worker)
(() => {
  let ok = false;
  ...
  // IMPORTANTE: não defina workerSrc e nem workerPort aqui.
  // Deixe o getDocument com disableWorker: true (abaixo) cuidar.
})();

// 2) módulos do parser (seus arquivos diretos, sem index.js)
try {
...
function reconstructLines(tc){
  ...
  for (const item of (tc.items||[])) {
    ...
    const x = tr[4], y = tr[5], s = item.str || '';
...
async function extractPdf(buf){
  ...
  // CHAVE: desliga o worker interno do pdf.js
